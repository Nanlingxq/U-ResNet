...
def ModelEval(model, test_loader, Epoch, train_on_gpu = train_on_gpu, target_size=100):
    test_loss = 0.0
    class_correct = list(0. for i in range(target_size))
    class_total = list(0. for i in range(target_size))
...
train_data = datasets.CIFAR100('./CIFAR100', train=True, download=True, transform=transform)
test_data = datasets.CIFAR100('./CIFAR100', train=False, download=True, transform=transform)
...
# model = NlNet_CIFAR.NlNet(num_classes=100, in_channels=3).to(device)
...
for epoch in range(1, Epochs + 1):
    ...
    accuracy = ModelEval(model, test_loader, epoch, 100)
    ...
    if valid_loss <= valid_loss_min:
        ...
        torch.save(model.state_dict(), f'./CIFAR100_Result/model_cifar_{model.name}.pt')
        valid_loss_min = valid_loss

    pd.DataFrame(train_result).to_excel(f'./CIFAR100_Result/{model.name}-CIFAR100-TrainResult.xlsx', index=False)
