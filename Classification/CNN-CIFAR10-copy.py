...
def ModelEval(model, test_loader, Epoch, train_on_gpu = train_on_gpu, criterion = None, train_loss = None):
    test_loss = 0.0
...
models = [
    Block.LeNet(in_channel=3, features=10).to(device),
    Block.BottleCNN(in_channel=3, features=10).to(device),
    Block.AlexNet(in_channels=3, features=10).to(device),
    Block.CNN(inchannels=3, features=10).to(device),
    Block.GoogLeNet(num_classes=10, aux_logits=False).to(device)
]



def train(model, initial_learning_rate):
    ...
    for epoch in range(1, Epochs + 1):
        ...
        print(model.name,'  Epoch: {}/{} \tTraining Loss: {:.6f} \tValidation Loss: {:.6f}'.format(
            epoch, Epochs, train_loss, valid_loss))

        accuracy = ModelEval(model, test_loader, epoch, train_on_gpu, criterion, train_loss)
