...
from modules import *
# from densenet import model as denseNet
import pandas as pd
# from vggnet import model as VggNet
# from resnest_MiniImageNet import resnest
...
model = NlNet_MiniImageNet.NlNet(num_classes=num_classes, in_channels=3).to(device)
# model = ShuffleNet.ShuffleNet_g8().to(device)
# model = denseNet.DenseNet().to(device)
# model = VggNet.vgg().to(device)
# model = resnest.resnet50(pretrained=False).to(device)
# model = EffecientNetv2.efficientnetv2_s(num_classes=100).to(device)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):
        ...
        if accuracy_top1 >= top:
            top = accuracy_top1
            torch.save(model.state_dict(), f'./MiniImageNetResult/model_ImageNet_{model.name}.pt')
