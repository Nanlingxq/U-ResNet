...
class SegmentationHead(nn.Module):
    def __init__(self, feature_channels, num_classes):
        super().__init__()
        # 使用实际的骨干网络输出通道数 [112, 224, 448, 640]
        ...
        # 注意拼接后的维度：224 + 256 = 480 → 448 + 256 = 704
    ...
    def forward(self, features):
        f0, f1, f2, f3 = features  # 特征图尺度递减
        
        # 从最深层的特征开始处理 (f3: 1/32分辨率)
        x = self.conv3(f3)  # [B, 256, H/32, W/32]
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)  # 上采样2倍 → 1/16

        # 与第三阶段特征f2拼接
        x = torch.cat([x, f2], dim=1)  # [B, 448+256=704, H/16, W/16]
        x = self.conv2(x)  # [B, 256, H/16, W/16]
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)  # 上采样2倍 → 1/8

        # 与第二阶段特征f1拼接
        x = torch.cat([x, f1], dim=1)  # [B, 224+256=480, H/8, W/8]
        x = self.conv1(x)  # [B, 256, H/8, W/8]
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)  # 上采样2倍 → 1/4

        # 与第一阶段特征f0拼接
        x = torch.cat([x, f0], dim=1)  # [B, 112+256=368, H/4, W/4]
        x = self.conv0(x)  # [B, 256, H/4, W/4]
...
class RMT_Segmentation(nn.Module):
    def __init__(self, num_classes=19, **kwargs):
        ...
        # 使用骨干网络的实际输出通道数 [112, 224, 448, 640]
        self.seg_head = SegmentationHead(
            feature_channels=kwargs['embed_dims'],  # 关键修改
