...
class UperHead(nn.Module):
    def __init__(self, in_channels, num_classes, pool_scales=(1, 2, 3, 6)):
        super().__init__()
        # 确保输入通道数匹配4个stage输出
        assert len(in_channels) == 4, "UperHead需要4个输入特征图"
        
        # PPM模块 - 处理最后一个stage的特征
        ...
        # PPM拼接后的降维卷积
        self.ppm_concat = nn.Sequential(
            nn.Conv2d(512 * len(pool_scales), 512, kernel_size=1, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True)
        )
        ...
        for ch in in_channels:  # 所有stage
            self.fpn.append(nn.Sequential(
                nn.Conv2d(ch, 512, kernel_size=1, bias=False),
                nn.BatchNorm2d(512),
                nn.ReLU(inplace=True)
            ))
    ...
    def forward(self, features):
        # 输入features: [stage0, stage1, stage2, stage3] 共4个特征图
        ppm_out = []
        for ppm in self.ppm:
            # 处理最后一个特征图(stage3)
            resized = F.interpolate(
                ppm(features[-1]),
                size=features[-1].shape[2:],  # 保持原始分辨率
                mode='bilinear',
                align_corners=False
            )
            ppm_out.append(resized)
        
        # 拼接并降维
        ppm_out = torch.cat(ppm_out, dim=1)
        ppm_out = self.ppm_concat(ppm_out)
        
        # 处理各stage特征
        fpn_features = []
        for i in range(len(features)):
            fpn_features.append(self.fpn[i](features[i]))
        
        # 替换最后一个特征图为PPM处理后的特征
        fpn_features[-1] = ppm_out
        
        # 自顶向下融合
        fused = fpn_features[-1]
        for i in range(len(fpn_features) - 2, -1, -1):
            # 计算需要上采样的倍数
            scale_factor = features[i].shape[2] / fused.shape[2]
            fused = fpn_features[i] + F.interpolate(
                fused,
        ...
        # 上采样回原始分辨率
        out = self.conv_seg(self.drop(fused))
        out = F.interpolate(out, scale_factor=4, mode='bilinear', align_corners=False)
        return out

class InceptionNeXtForSegmentation(nn.Module):
    def __init__(self, backbone, num_classes):
        super().__init__()
        self.backbone = backbone
        self.name = "InceptionNeXt_Seg"
        self.head = UperHead(
            in_channels=backbone.dims,  # 直接使用各stage的输出通道数
            num_classes=num_classes
        )
    
    def forward(self, x):
        features = self.backbone.forward_features(x)
        # 跳过stem输出，使用4个stage的特征
