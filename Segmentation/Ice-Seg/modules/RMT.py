...
class RetNetRelPos2d(nn.Module):
    ...
    def generate_2d_decay(self, H: int, W: int):
        ...
        # 修改此行，添加indexing参数 ↓
        grid = torch.meshgrid([index_h, index_w], indexing='ij')  # 明确索引方式
...
class RMT(nn.Module):
    def __init__(self, in_chans=3, out_indices=(0, 1, 2, 3),
                 embed_dims=[96, 192, 384, 768], depths=[4, 8, 25, 8], num_heads=[7, 7, 14, 20],
                 init_values=[2, 2, 2, 2], heads_ranges=[6, 6, 6, 6], mlp_ratios=[4, 4, 3, 3], drop_path_rate=0.1, norm_layer=nn.LayerNorm,
                 patch_norm=True, use_checkpoint=False, chunkwise_recurrents=[True, True, True, False], projection=1024,
                 layerscales=[False, False, True, True], layer_init_values=1e-6, norm_eval=True):
        super().__init__()
...
class SegmentationHead(nn.Module):
    def __init__(self, feature_channels, num_classes):
        super().__init__()
        # 参数说明：
        # feature_channels: 各阶段特征图的通道数列表，例如[96, 192, 384, 768]
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(feature_channels[2] + 256, 256, 3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )

        self.conv1 = nn.Sequential(
            nn.Conv2d(feature_channels[1] + 256, 256, 3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )

        self.conv0 = nn.Sequential(
            nn.Conv2d(feature_channels[0] + 256, 256, 3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
    ...
    def forward(self, features):
        # features包含四个不同尺度的特征图（从RMT返回）
        f0, f1, f2, f3 = features

        # 从最深层的特征开始处理
        x = self.conv3(f3)  # [2,256,20,20]
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)  # 40x40

        # 与第三阶段特征融合
        x = torch.cat([x, f2], dim=1)  # [2,384+256=640,40,40]
        x = self.conv2(x)  # [2,256,40,40]
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)  # 80x80

        # 与第二阶段特征融合
        x = torch.cat([x, f1], dim=1)  # [2,192+256=448,80,80]
        x = self.conv1(x)  # [2,256,80,80]
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)  # 160x160

        # 与第一阶段特征融合
        x = torch.cat([x, f0], dim=1)  # [2,96+256=352,160,160]
        x = self.conv0(x)  # [2,256,160,160]

        # 上采样到原始尺寸（假设输入是640x640，此时特征图是160x160）
        x = self.up4x(x)  # 640x640
        x = self.final_conv(x)  # [2,num_classes,640,640]

        return x


class RMT_Segmentation(nn.Module):
    def __init__(self, num_classes=1, **kwargs):
        super().__init__()
        # 初始化RMT骨干网络
        self.backbone = RMT(**kwargs)
        self.name = "RMT"
        # 初始化分割头
    ...
    def forward(self, x):
        # 获取骨干网络的多尺度特征
        features = self.backbone(x)  # 返回四个特征图的元组
